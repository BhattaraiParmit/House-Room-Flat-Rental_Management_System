...
// let province_select_box = document.getElementById('id_province')
...
// ================================Removing error from input sections=========================================

document.getElementById('id_phone').addEventListener('click', ()=>{
    document.getElementById('phone-err-mssg-box').style.display = 'none'
    document.querySelector('.phone-input').attributeStyleMap.clear()

})


document.getElementById('id_province').addEventListener('click', (btn)=>{
    document.getElementById('id_province').attributeStyleMap.clear()
    document.getElementById('id_city').attributeStyleMap.clear()
})


document.getElementById('id_area').addEventListener('click', ()=>{
    document.querySelector('.square-input').attributeStyleMap.clear()
})


document.getElementById('id_description').addEventListener('click', ()=>{
    document.getElementById('id_description').attributeStyleMap.clear()
})


document.getElementById('id_price').addEventListener('click', ()=>{
    document.querySelector('.price-input').attributeStyleMap.clear()
})
...
// ================================Removing error from input sections end=========================================




province.addEventListener('change', ()=>{
    // value = province_select_box.value
    console.log(province.value)
    province_value = province.options[province.selectedIndex].value
    province_id = province.options[province.selectedIndex].dataset.value
    console.log(province_id)
    console.log(province_value)
    displayCity(province_id)
})
...
footer.addEventListener('click', (e) =>{    
    ...
    let get_started_btn = e.target.closest('.get-started-btn')
    ...
    if(get_started_btn){
        e.preventDefault()
        get_started_btn.classList.remove('get-started-btn')
        get_started_btn.classList.add('form2-btn')
        formStepNum++
        updateFormStep(formStepNum)
        updateProgressBar(0)
...
function checkFormValidation(btn, label){
    ...
    if(label == 'form2-btn'){
        // province_value = province.options[province.selectedIndex].dataset.value
        province_value = province.value
        // city_value = city_select_box.options[city_select_box.selectedIndex].dataset.value
        ...
        if(phone.value == ''){
            ...
            document.getElementById('phone-err-mssg-box').style.display = 'block'
            document.getElementById('phone-error-mssg').innerText= 'Please input your phone number'

        }else{
            console.log("not empty")

            phone_Number = document.getElementById('id_phone').value

            if(phone_Number.length < 10){
                document.getElementById('phone-err-mssg-box').style.display = 'block'
                document.getElementById('phone-error-mssg').innerText= 'Your phone number is not valid'
                return   
            }
...
imageUploadBtn.addEventListener('click', () =>{
    Input.click()
    document.querySelector('.error-mssg-box').attributeStyleMap.clear()
    // document.querySelector('.clear').attributeStyleMap.clear()
})
...
videoInput.addEventListener('change', function(){
    ...
    console.log("jjjjjjjjjjjjjjjjjjjjj", videos)
...
picture_section.addEventListener('click', function(e){
    ...
    if(deleteEl){
        ...
        if(deleteEl.parentNode.classList.contains('coverImage')){
            ...
            files.splice(deleteEl.dataset.number, 1)
            ...
            console.log('-----', deleteEl.dataset.number)     
            files.splice(deleteEl.dataset.number, 1)
...
post_property_form.addEventListener('submit', (e) =>{
    ...
    room_data = { 
                ...
                'pet': pet_value, 'parking': parking_value,
...
function displayCity(province_id){


    var url = "/getCityFromProvinceName/"
    ...
    fetch(url, {
        ...
        body: JSON.stringify({'id': province_id})
    ...
    .then((data) =>{
        ...
        for(var i = 0; i< city.length; i++){
            city_select_box.innerHTML += `<option value="${city[i].fields.name}" data-value = "${city[i].pk}">${city[i].fields.name}</option>`
        }
...
function displayForm11Content(){
    ...
    form11Image.src = URL.createObjectURL(cover_photo[0])
